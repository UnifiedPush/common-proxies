...
| Transparent Draft 4 GET Payload    | gateway.TransparentDraft4.GetPayloadString | UP_GATEWAY_TRANSPARENT_DRAFT4_GETPAYLOAD     | string              | Payload for GET requests - GET requests should not be proxied, and even in that case, the default is `{"unifiedpush":{"version":1}}`
